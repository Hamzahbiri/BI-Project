...
export default {
  mounted() {
    // Fetch MDX data from your SSAS server
    axios.get('http://localhost:3000/executeMdx', {
      params: {
        mdxQuery: 'Your MDX Query Here',
      },
    })
    .then(response => {
      // Extract data from the response
      const mdxData = response.data;

      // Use the mdxData to update your charts
      this.updateCharts(mdxData);
    })
    .catch(error => {
  ...
  methods: {
    updateCharts(mdxData) {
      // Update your charts using the mdxData
      // For example, if mdxData is an array of values, you might use it like:
      const newData = {
      ...
      // Assuming this.$refs.myChart is the reference to your chart canvas
