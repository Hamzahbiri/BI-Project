...
export default {
  ...
  methods: {
    ...
    updateChart() {
      const filteredLabels = this.filterLabels(this.originalData.labels, this.filterTerm);
      this.chartData.labels = filteredLabels;
      this.chartData.datasets[0].data = this.filterData(this.originalData.datasets[0].data, filteredLabels);
      this.myChart.update();
    },
    filterLabels(labels, term) {
      return labels.filter(label => label.toLowerCase().includes(term.toLowerCase()));
    },
    filterData(data, labels) {
      return data.filter((_, index) => labels.includes(this.originalData.labels[index]));
    },
    updateChartData() {
      this.chartData = JSON.parse(JSON.stringify(this.originalData)); // Deep copy to make it reactive
    },
  },
};
</script>
