...
<div class="card w-75 m-2 p-2">
  <table>
    ...
    <div class="box">
      ...
      <div class="legend">
        <div v-for="(color, index) in legendColors" :key="index" @click="toggleLegend(index)">
...
export default {
  mounted() {
    ...
    const dataEmail = {
  legendColors: ['rgba(255, 99, 132, 1)', 'rgba(54, 162, 235, 1)', 'rgba(255, 206, 86, 1)'],
  legendLabels: ["Emails Envoyés", "Emails Reçus", "Emails Expédiés"],
    ...
    legend: {
      ...
      labels: {
        filter: function (item, chart) {
          // Filter the legend items based on user selection
          return item.datasetIndex === chart.legendColorIndex;
        }
      }
    }
  }
...
// Add an event listener to the canvas for color selection
this.$refs.gaugeChart.addEventListener('click', function (event) {
  const activePoint = gaugeChart.getElementsAtEventForMode(event, 'nearest', { intersect: true }, true);

  if (activePoint.length > 0) {
    const datasetIndex = activePoint[0].datasetIndex;
    gaugeChart.legendColorIndex = (gaugeChart.legendColorIndex === datasetIndex) ? null : datasetIndex;
    gaugeChart.update();
  }
});
  }}
